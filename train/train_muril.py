"""
MuRIL fine-tune for hate-speech detection (HASOC 2019 EN + HI, task_1: HOF/NOT)
...
Run:
    conda run -n test python train/train_muril.py

Outputs:
    model/muril_model/      (HuggingFace saved model + tokenizer)
    model/muril_report.json
    model/plots/muril_training_history.png
"""
...
train_df = pd.concat([load_tsv(p) for p in [EN_TRAIN, HI_TRAIN] if os.path.exists(p)], ignore_index=True)
test_df  = pd.concat([load_tsv(p) for p in [EN_TEST,  HI_TEST]  if os.path.exists(p)], ignore_index=True)
...
print(f"\nFine-tuning MuRIL ({EPOCHS} epochs)...\n")
