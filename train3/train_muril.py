...
Outputs:
    model3/muril/           (HuggingFace saved model + tokenizer)
    model3/muril/report.json
    model3/plots/muril_training_history.png
...
MAX_LEN     = 64         # reduced from 128 (HASOC text is short)
BATCH_SIZE  = 16
EPOCHS      = 10         # reduced from 20 (best epoch is ~7)
LR          = 1e-5        # lower learning rate for better fine-tuning
WARMUP_FRAC = 0.15        # more warmup steps for stable learning
EARLY_STOP_PATIENCE = 3   # early stopping if no improvement for 3 epochs
SEED        = 42
...
# ── minimal cleaning using prep2.preprocess_muril() ─────────────────────────────
def clean(text: str) -> str:
    """Use prep2.preprocess_muril() for minimal preprocessing (transformer-friendly)."""
...
combined_train_df = pd.concat([load_any(p) for p in train_paths if os.path.exists(p)], ignore_index=True)
...
train_df, val_df = train_test_split(
    combined_train_df, 
    test_size=0.1, 
    stratify=combined_train_df['task_1'],
    random_state=SEED
)

print(f"  Train: {len(train_df)}  HOF={(train_df['task_1']=='HOF').sum()}  NOT={(train_df['task_1']=='NOT').sum()}")
print(f"  Val  : {len(val_df)}   HOF={(val_df['task_1']=='HOF').sum()}  NOT={(val_df['task_1']=='NOT').sum()}")
...
val_loader   = DataLoader(HateDataset(val_df),   batch_size=BATCH_SIZE, shuffle=False, num_workers=0)
...
# class weights for imbalance (FIXED: proper inverse weighting)
hof  = (train_df['task_1'] == 'HOF').sum()
not_ = (train_df['task_1'] == 'NOT').sum()
class_weights = torch.tensor([
    len(train_df) / (2 * not_),   # weight for NOT (index 0)
    len(train_df) / (2 * hof)     # weight for HOF (index 1)
], dtype=torch.float).to(DEVICE)
print(f"\nClass weights: NOT={class_weights[0]:.4f}, HOF={class_weights[1]:.4f}")
...
print(f"{'Epoch':>5}  {'Train Loss':>10}  {'Train Acc':>9}  {'Val Loss':>8}  {'Val Acc':>7}  {'F1-mac':>6}  {'ES':>3}")
print("-" * 68)
...
for epoch in range(1, EPOCHS + 1):
    ...
    # eval on VALIDATION set (not test)
    model.eval()
    ...
    print(f"{epoch:>5}  {t_loss:>10.4f}  {t_acc:>9.4f}  {v_loss:>8.4f}  {v_acc:>7.4f}  {v_f1:>6.4f}  {early_stop_count:>3}")

    if v_f1 > best_f1:
        best_f1, best_epoch = v_f1, epoch
        early_stop_count = 0  # Reset counter on improvement
    ...
    if early_stop_count >= EARLY_STOP_PATIENCE:
        print(f"\n⏹️  Early stopping triggered at epoch {epoch} (no improvement for {EARLY_STOP_PATIENCE} epochs)")
        break
