...
Trains on combined data from both years for improved coverage.
...
Run:
    python train2/train_model.py

Outputs:
    model3/lr/model.pkl        (full Pipeline: FeatureUnion → LogisticRegression)
    model3/lr/vectorizer.pkl   (FeatureUnion only, kept for backward compatibility)
    model3/lr/report.json
    model3/plots/lr_*.png
"""
...
# ── preprocessing using prep2.py (model-specific) ───────────────────────────────
def clean(text: str) -> str:
    """Use prep2.preprocess_lr() for enhanced LR preprocessing with Hinglish normalization."""
    return preprocess_lr(text)

# ── loaders ───────────────────────────────────────────────────────────────────
def _normalise(df: pd.DataFrame) -> pd.DataFrame:
    """Rename task1/task_1 → task_1, keep text + task_1, clean."""
    df.columns = df.columns.str.strip()
    # handle both 'task1' (2020) and 'task_1' (2019)
...
fig.suptitle("Class Distribution (HASOC 2019+2020)", fontsize=14, fontweight="bold")
plt.tight_layout()
...
ax.set_title("Token Length Distribution (train, 2019+2020)", fontsize=12)
ax.legend()
ax.spines[["top", "right"]].set_visible(False)
plt.tight_layout()
